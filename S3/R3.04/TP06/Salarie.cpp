...
Salarie::Salarie(const string &nom, const string &numeroSS, const float &salaireMensuel): m_salaireMensuel(ObjetContraint<float>(0, 0, 0)) {
    this->setNom(nom);
...
const string &Salarie::getNom() const noexcept {
    string upper = "";
    std::string::const_iterator it = this->m_nom.begin();
    while (it != this->m_nom.end()) {
        upper += toupper(*it);
    }
    return upper;
}
